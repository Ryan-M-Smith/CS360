...
/**
 * @brief 		Perform an iterative quicksort on an array.
 * 
 * @param array The array to sort
 * @param size 	The size of the array
 * 
 * @see 		Reference for the interative algorithm from [GeeksforGeeks](https://www.geeksforgeeks.org/dsa/iterative-quick-sort/)
 */
void quicksort(int* array, int size) {
	std::stack<indices> stack;

	// Push the initial bounds
	stack.push({0, size - 1});
	...
	while (!stack.empty()) {
		...
		// If there are elements on the left side of the pivot, look at the left subarray
		if (pivot - 1 > left) {
			stack.push({left, pivot - 1});
		}

		// If there are elements on the right side of the pivot, look at the right subarray
		if (pivot + 1 < right) {
